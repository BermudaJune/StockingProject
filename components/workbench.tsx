...
输出：单张全身主图`;
const SINGLE_STEP_C_PROMPT = `基于上一张图片进行编辑。

【绝对锁定】
- 上一张图中的人物、腿部、袜子、鞋子、服装必须完全保持不变
...
export function Workbench({ initialTemplates }: Props) {
  ...
  async function handlePickSingleInputDirectory() {
    try {
      ...
      if (!granted) {
        throw new Error("未获得单组输入目录写入权限，无法回写 result_A、result_B 与 result_0 到 result_5");
      }
  ...
  async function handleStartGeneration() {
    ...
    if (!singleInputDirectory) {
      setErrorMessage("单组模式请先选择“单组输入目录”，系统会从该目录读取输入图并回写 result_A、result_B 与 result_0 到 result_5");
      return;
    }
    ...
    try {
      const stepAPrompt = buildSingleStepAPrompt(template.mainPrompt);
      const stepBPrompt = buildSingleStepBPrompt(template.stepOnePrompt);
      const stepCPrompt = buildSingleStepCPrompt(template.stepThreePrompt);
      ...
      const expectedFiles = ["result_A.png", "result_B.png", "result_0.png", "result_1.png", "result_2.png", "result_3.png", "result_4.png", "result_5.png"];

      if (continueFromExisting) {
        const missing = await listMissingResultFiles(singleInputDirectory.handle, expectedFiles);
        if (missing.length === 0) {
          setStatusMessage("单组目录已存在 result_A、result_B、result_0 到 result_5，已跳过。");
          return;
        }
      }
      ...
      setStatusMessage(
        "单组四步流程完成，已写入该输入目录：result_A.png、result_B.png 与 result_0.png 到 result_5.png。"
      );
    } catch (error) {
  ...
  async function handlePickBatchInputDirectory() {
    try {
      ...
      const granted = await ensureDirectoryPermission(directoryHandle, "readwrite");
      if (!granted) {
        throw new Error("未获得输入目录写入权限，无法回写 result_A、result_B 与 result_0 到 result_5");
      }
  ...
  async function handleStartBatchGeneration() {
    ...
    try {
      ...
      appendBatchLog(`按文件名关键词识别角色：产品=“款/产品”，鞋子=“鞋”，背景=“背景”，服装=“服装”；模型输入顺序：产品 -> 鞋子 -> 服装 -> 背景。`);
      ...
      for (let i = 0; i < groups.length; i++) {
        ...
        try {
          ...
          if (continueFromExisting) {
            ...
            if (missing.length === 0) {
              success += 1;
              appendBatchLog(`[${group.name}] 已存在 result_A、result_B、result_0~5，跳过。`);
              continue;
            }
          }
          ...
          success += 1;
          appendBatchLog(`[${group.name}] 成功，已写入 result_A.png、result_B.png 与 result_0.png 到 result_5.png`);
        } catch (error) {
          if (isAbortLikeError(error)) {
            throw error;
          }
  ...
  return (
    <main className="page-shell">
      <section className="hero">
        ...
        <span className="hero-badge">四图主图系统 · 四步批处理</span>
        <h1>输入产品/模特/场景，生成展示图</h1>
        <p>批量模式会按每个子文件夹直接回写 result_A、result_B、result_0 到 result_5，不改变原文件结构。</p>
      </section>
          ...
          <section className="panel panel-strong">
            <div className="section">
              <div className="section-header">
                <div>
                  <h2>批量生图</h2>
                  <p>每个子文件夹作为一组，按文件名关键词识别角色：产品=“款/产品”，鞋子=“鞋”，背景=“背景”，服装=“服装”；模型输入顺序为“产品-鞋子-服装-背景”。</p>
                </div>
              ...
              <div className="stack">
                ...
                <div className="note-card tiny">输出规则：结果直接写回各子文件夹，命名为 result_A.png、result_B.png、result_0.png 到 result_5.png</div>
            ...
            <div className="section">
              <div className="section-header">
                <div>
                  <h2>单组输入</h2>
                  <p>单组生成会从“单组输入目录”读取图片并回写 result_A、result_B 与 result_0 到 result_5。上传区仅用于预览与对照。</p>
                </div>
              </div>
            ...
            <div className="section">
              <div className="section-header">
                <div>
                  <h3>参数与模板</h3>
                  <p>左下角可直接编辑 A步（腿袜鞋）、B步（加服装）、C步（加背景）与 D步（裂变）提示词。</p>
                </div>
              </div>
              <div className="toolbar">
                <div className="field">
              ...
              <div className="stack">
                <div className="field">
                  <label htmlFor="mainPrompt">A步提示词（产品+鞋子 生成 result_A）</label>
                ...
                <div className="field">
                  <label htmlFor="stepOnePrompt">B步提示词（基于 result_A + 服装 生成 result_B）</label>
                ...
                <div className="field">
                  <label htmlFor="stepThreePrompt">C步提示词（基于 result_B + 背景 生成 result_0）</label>
                ...
                <div className="field">
                  <label htmlFor="stepTwoPrompt">D步裂变提示词（基于 result_0 裂变 result_1~5）</label>
...
function buildSingleStepAPrompt(mainPrompt: string): string {
  const tweak = mainPrompt.trim();
  const basePrompt = tweak ? `${SINGLE_STEP_A_PROMPT}\n\n${tweak}` : SINGLE_STEP_A_PROMPT;
  if (basePrompt.includes("Step A 袜子硬约束")) {
    return basePrompt;
  }
  return `${basePrompt}\n\n${STEP_A_SOCK_LOCK}`;
}

function buildSingleStepBPrompt(stepOnePrompt: string): string {
  const tweak = stepOnePrompt.trim();
  if (!tweak) {
...
function buildSingleStepCPrompt(stepThreePrompt: string): string {
  const tweak = stepThreePrompt.trim();
  if (!tweak) {
    return SINGLE_STEP_C_PROMPT;
  }
  return `${SINGLE_STEP_C_PROMPT}\n\n${tweak}`;
}
